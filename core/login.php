...
<html>
	<head>
		<title> C.O.R.E Login </title>
		<link rel="stylesheet" type="text/css" href="./style/login_style.css" />
	</head>
	<body>
<?php
	setcookie("clogin","false",time()+60*60*24*1,"/",".umunc.net");
	if ($_POST["posted"]=="true"){
		...
		if ($_POST["password"]==$pwd){
			setcookie("clogin","pass",time()+60*60*24*1,"/",".umunc.net");
			setcookie("id","$id",time()+60*60*24*1,"/",".umunc.net");
			setcookie("user","$user",time()+60*60*24*1,"/",".umunc.net");
			setcookie("prio","$prio",time()+60*60*24*1,"/",".umunc.net");
